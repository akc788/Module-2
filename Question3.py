...
class ReservedMemory:
    """
    A class to reserve and handle a contiguous area of memory.
    """
    ...
    def copy(self, mem_source: ReservedMemory, count: int = None,
             source_index: int = 0, destination_index: int = 0) -> None:
        if count is None:
...
class IntArray:
    """
    Integer array implemented using static memory.
    """
    ...
    def __repr__(self) -> str:
        ...
        return f"IntArray ({self._size} element{plural}): [{', '.join(map(str, self))}]"
    ...
    def __setitem__(self, k: int, val: int) -> None:
        if not self._min_val <= val <= self._max_val:
            raise TypeError("Value out of bounds")
    ...
    def insert(self, index: int, val: int) -> None:
        ...
        if self._resmem and index > 0:
            new_mem.copy(self._resmem, index * self._bytes_per_element, 0, 0)
    ...
    def search(self, value: int) -> int:
        """
        Search for the first occurrence of 'value' in the array.
